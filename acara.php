...
<head>
    ...
    <title>Daftar Acara Siaran</title>
...
<body>
  ...
  <div class="container-fluid">
  	...
  	<?php  
          echo'<a href="tambah_acara.php" class="btn btn-success"  title="tambah data Acara">Tambah</i></a>';
      ?>
  </div>
</nav>
        ...
        <table id="example" class="table table-striped" style="width:100%; background-color:white;" border="1">
            <thead>
                <tr>
                    ...
                    <th scope="col">Hari Siaran</th>
                </tr>
            </thead>
            <tbody>
            <?php include("koneksi.php"); ?>
    <?php
              $cari = "SELECT * FROM daftar_acara_siaran";
              $query = mysqli_query($koneksi,$cari);
              $jlhData = mysqli_num_rows($query);
          if($jlhData == 0){
              echo"<tr><td colspan=5>Acara Siaran Tidak Ada</td></tr>";
          } else {
            ?>
            <?php while ($row = mysqli_fetch_array($query)) : ?>
  <tr>
      <?php
        echo "<td><a class='btn btn-sm btn-success' href='edit_acara.php?id_acara=".$row[0]."'>Edit</a>| 
                <a class='btn btn-sm btn-danger' href = 'hapus_acara.php?id_acara=".$row[0]."'";?> onclick="return confirm('yakin ingin menghapus jadwal ini')">Hapus</a></td>
      <td><?= $row["nama_acara_siaran"] ?></td>
      <td><?= $row["deskripsi"] ?></td>
      <td><?= $row["hari_siaran"] ?></td>
      </tr>
  <?php endwhile; ?>   
<?php } ?>
                <tr>
        ...
        <?php 
echo'<a href="cetak_acara.php" target="_blank"><button class="btn btn-success">CETAK</button></a>';
?>
    </div>
</div>
