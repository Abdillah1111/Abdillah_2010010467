...
<body>
  ...
  <div class="container-fluid">
  	...
  	<?php  
          echo'<a href="tambah_penyiar.php" class="btn btn-success"  title="tambah data">Tambah</i></a>';
      ?>
  </div>
</nav>
    ...
    <?php
              $cari = "SELECT * FROM data_penyiar";
              $query = mysqli_query($koneksi,$cari);
              $jlhData = mysqli_num_rows($query);
          if($jlhData == 0){
              echo"<tr><td colspan=5>Data Penyiar Tidak Ada</td></tr>";
          } else {
            ?>
            <?php while ($row = mysqli_fetch_array($query)) : ?>
  <tr>
      <?php
        echo "<td><a class='btn btn-sm btn-success' href='edit_penyiar.php?username=".$row[0]."'>Edit</a>| 
                <a class='btn btn-sm btn-danger' href = 'hapus_penyiar.php?username=".$row[0]."'";?> onclick="return confirm('yakin ingin menghapus Data Penyiar ini?')">Hapus</a></td>
      <td><?= $row["nama_penyiar"] ?></td>
      <td><?= $row["kelamin_penyiar"] ?></td>
      <td><?= $row["tanggal_lahir_penyiar"] ?></td>
      <td><?= $row["no_telpon_penyiar"] ?></td>
      </tr>
  <?php endwhile; ?>   
<?php } ?>
                <tr>
        ...
        <?php 
echo'<a href="cetak_penyiar.php" target="_blank"><button class="btn btn-success">CETAK</button></a>';
?>
    </div>
</div>
